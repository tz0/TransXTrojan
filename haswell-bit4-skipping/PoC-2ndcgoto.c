...
This source code (PoC-cgoto.c) shows a Proof-of-Concept (PoC) example to 
demonstrate the "Skipping" mechanism transient attacks using an indirect jump.

In this example, do_1/2 and do_3 under cgoto statement can access user_dat 
with either secret value (in case 0), or nonsecret value (in case 2).  Since 
case selector sw is fixed to point to case 2. The execution of computed goto 
should always take place at nowhere but do_3 and accesses user_data with non 
secret value as  user_dat[position_nonsecret*256].  However, during resolving 
the indirect jump inside of the cgoto statement, "skipping" mechanism advances 
...
// // int access_udata(unsigned char* code, int initval) {
// int access_udata(int * code) {
//     static void* dispatch_table[] = {&&do_0, &&do_1, &&do_2, &&do_3, &&do_4, &&do_5, &&do_6};
...
//         do_1:
//             val += 1;
//             position = position_secret; // assigns a index variable with a nonsecret value.
...
int main(int argc, char ** argv) {
  ...
  int code[] = {2,3,6};
  // cgoto act like another switch when -O0
  while(1){
